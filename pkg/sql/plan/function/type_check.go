...
func initTypeCheckRelated() {
	...
	var convertRuleForBinaryTable [][4]types.T // left-input, right-input, left-target, right-target
	{
		...
		for _, typ1 := range decimals {
			...
			for _, typ2 := range floats {
				convertRuleForBinaryTable = append(convertRuleForBinaryTable, [4]types.T{typ1, typ2, typ2, typ2})
				convertRuleForBinaryTable = append(convertRuleForBinaryTable, [4]types.T{typ2, typ1, typ2, typ2})
			}
...

		{
			...
			for i := range decimals {
				...
				convertRuleForBinaryTable = append(convertRuleForBinaryTable, [4]types.T{typ, decimals[i], decimals[i], decimals[i]})
			}
		}
	...
	var convertRuleForBinaryTable2 [][4]types.T
	{
		...
		for i := range numbers {
			...
			for j := range decimals {
				convertRuleForBinaryTable2 = append(convertRuleForBinaryTable2, [4]types.T{numbers[i], decimals[j], types.T_decimal128, types.T_decimal128})
				convertRuleForBinaryTable2 = append(convertRuleForBinaryTable2, [4]types.T{decimals[j], numbers[i], types.T_decimal128, types.T_decimal128})
			}
			for j := range floats {
		...
		}
		{
			...
			for i := range decimals {
				...
				convertRuleForBinaryTable2 = append(convertRuleForBinaryTable2, [4]types.T{typ, decimals[i], decimals[i], decimals[i]})
			}
		}
		{
			typ := types.T_datetime
		...
		}
		{
			typ := types.T_bool
			for i := range ints {
				convertRuleForBinaryTable2 = append(convertRuleForBinaryTable2, [4]types.T{ints[i], typ, ints[i], ints[i]})
				convertRuleForBinaryTable2 = append(convertRuleForBinaryTable2, [4]types.T{typ, ints[i], ints[i], ints[i]})
			}
			for i := range uints {
				convertRuleForBinaryTable2 = append(convertRuleForBinaryTable2, [4]types.T{uints[i], typ, uints[i], uints[i]})
				convertRuleForBinaryTable2 = append(convertRuleForBinaryTable2, [4]types.T{typ, uints[i], uints[i], uints[i]})
			}
		}
